...
public class ExcelTools extends FileTools {
     ...
     * @throws IOException 
     */
    public static List<Map<String, String>> readExcel (InputStream inputStream, String originFileName, Map<String, Integer> keyMaps) throws FileException, IOException {
     ...
     * @throws IOException
     */
    public static ByteArrayOutputStream writeToXLS(List<String> heads, String[][] data, String sheetName, ByteArrayOutputStream out) throws IOException {
     ...
     * @throws IOException
     */
    public static ByteArrayOutputStream writeToXLSX(List<String> heads, List<List<Object>> data, String sheetName, ByteArrayOutputStream out) throws IOException {
            XSSFWorkbook wb = new XSSFWorkbook();
            XSSFSheet sheet1 = wb.createSheet(sheetName);
            XSSFRow row = sheet1.createRow(0);
            XSSFCellStyle style = wb.createCellStyle();
            ...
            int colLength = data.get(0).size();

            for (int n = 0; n < data.size(); n++) {
                row = sheet1.createRow(n + 1); 
                for (int m = 0; m < colLength; m++) {
                    cell = row.createCell(m);
                    cell.setCellValue(data.get(n).get(m).toString());
                    cell.setCellStyle(style);
                }
            }
            wb.write(out);
    ...
    /**
     * 07以后的版本
     * 将给定的表头和数据写入xlsx结尾的Excel，所有数据在多个sheet里
     */
    public static void writeToXLSX(ExcelRow heads, List<ExcelData> datas, String sheetName, ByteArrayOutputStream out) throws IOException {
    	SXSSFWorkbook wb = new SXSSFWorkbook(100); 
    	for(int num=0; num<datas.size(); num++) {
    		ExcelData data  = datas.get(num);
...
//        InputStream is = new FileInputStream(file);
    ...
    public static ResponseEntity<byte[]> export(ByteArrayOutputStream bOutputStream, String fileName) {
		try {
			fileName = new String((fileName+DateUtils.toString(new Date(), "yyyy-MM-dd")+".xlsx").getBytes(), "iso-8859-1");
    ...
    public static ResponseEntity<byte[]> exportXlsx(ByteArrayOutputStream bOutputStream, String fileName) {
		try {
			fileName = new String((fileName+DateUtils.toString(new Date(), "yyyy-MM-dd")+".xlsx").getBytes(), "iso-8859-1");
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
...
public static class ExcelException extends FileTools.FileException {
	...
	public static ExcelRow excelHeadersList(List<String> headers) {
		ExcelRow row = new ExcelRow();
