...
public class DateUtils extends org.apache.commons.lang.time.DateUtils {
	...
	public static List<String> getBetweenDates(Date start, Date end) {
	    ...
	    while (tempStart.before(tempEnd)) {
	        result.add(formatDate(tempStart.getTime(),"yyyy-MM-dd"));
...
//		System.out.println(a == b);
		List<String> result = getBetweenDates(parseDate("2017-11-1", "yyyy-MM-dd"),parseDate("2017-11-9", "yyyy-MM-dd"));
		result.stream().forEach(each -> {
			System.out.println(each);
		});
//		System.out.println(DateUtils.addYear(new Date(), 3));
	}
}
